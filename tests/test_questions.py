...
def test_post_question_is_actually_posting(client):
    ...
    assert not 'Question posted successfully' in response.data

# Testing if the question being posted is not repeated
def test_post_question_is_not_repeated(client):
    response = client.post('/api/v1/questions', data={ "title": "Title One", "content": "Lorem ipsum dolor sit amet"})
...
def test_post_question_is_not_missing_title(client):
    response = client.post('/api/v1/questions', data={ "title": "", "content": "Lorem ipsum dolor sit amet"})
    assert not 'Please fill the title' in response.data
